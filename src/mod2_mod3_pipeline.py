...
YOLO_NAME_TO_CLASS_ID = {
    "scab":    0,   # SCAB    → DISEASED (mod3: YOLO_CLASS_DISEASED = 1)
    "weeds":   1,   # Weeds   → WEED    (mod3: YOLO_CLASS_WEED = 2)
    "healthy": 2,   # healthy → HEALTHY (mod3: YOLO_CLASS_HEALTHY = 0)
}

# best.pt class_id → mod3 YOLO_CLASS_* eslemesi
# best.pt:  0=SCAB,   1=Weeds, 2=healthy
# mod3:     DISEASED=1, WEED=2, HEALTHY=0
BESTPT_TO_MOD3_CLASS = {
    0: 1,   # SCAB    → YOLO_CLASS_DISEASED
    1: 2,   # Weeds   → YOLO_CLASS_WEED
    2: 0,   # healthy → YOLO_CLASS_HEALTHY
}

BESTPT_CLASS_NAMES = {0: "SCAB", 1: "Weeds", 2: "healthy"}
...
def load_yolo(model_path: str):
    """best.pt'yi ultralytics YOLO ile yukle."""
    from ultralytics import YOLO
    log = logging.getLogger("pipeline.yolo")
    log.info(f"YOLO yukleniyor: {model_path}")
    t0 = time.time()
...
def main() -> int:
    ...
    if not args.no_vlm:
        ...
        # VLM yoksa monkey-patch: her zaman bypass-forced
        sys.path.insert(0, str(Path(__file__).parent / "vlm"))
